...
targetを外す派生は、移動先のWorkからそのWork自身への `requires_completion` を登録することがある（NGは再integrationから元のconfirmationへ必ず登録し、temporary moveはexecutorが決めた場合に登録する）。これはそのWorkを再び実行する前（戻る際）に確認するdependencyで、中断の無いcycleは登録からtarget削除までの間にdependencyを確認しない。そのため、派生の登録stageを記録した後・target削除を記録する前に中断した再実行は、Workを実行する前のdependency確認で、その派生自身が登録したそのWorkへの `requires_completion` だけを未充足として数えず、記録済みの派生を登録し直してtarget削除から続ける。数えないのは、このmutationの再実行で記録済みの派生を示せ（上の条件）、Workがそのcycleのtargetを持ったままで、cycleの最後の派生がtargetを外すもので、その登録stageが記録済みの最後のstageであり（target削除は未記録）、そのstageがその決定の作る登録と完全一致し（予約したrelation IDとpayloadを含む）、Projectがその関係を登録どおりに持つ場合の、その関係だけである。元からあるdependency、別の決定や他者が加えたもの、決定を示せないrecordのものは従来どおり確認して停止し、登録と一致しないstageはcycleの何も実行せず `reconcile_required` で停止する。targetを外さない派生の後の再実行は、この扱いを受けない。
