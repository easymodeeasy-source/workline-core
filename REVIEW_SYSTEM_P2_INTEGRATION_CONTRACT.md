...
| implementation blocker repair round 1 baseline = contract under repair | `de3681c94d9a0c8bf1ac8263513b5793c495a02e` — local `main` of a fresh GitHub clone, `origin/main` and GitHub `refs/heads/main` verified equal, working tree clean; `src/`, `tests/`, Skills and `registry.md` still byte-identical to `b78be1c`, and the contract text still byte-identical to the reviewed checkpoint `6004669` (this commit adds only the readiness verdict record, §32). A P2 implementation attempt on this baseline met six contradictions in the contract and stopped before its first push; they are repaired here (§33.8). The repair changes this document only, and it lands no implementation |
...
| canonical generation 1 | a Run's generation 1 committed at HEAD and reading back under the P1 reader, with its candidate snapshot and task input: the accepted Review Run exists (§11.6, §12.3) |
| Phase-entry continuation mode | how a review-v1 Phase entry is admitted once the slot's pending review-v1 Phase-entry planning mutation holds a Run with a canonical generation 1: the mutable facts that decide whether a *new* entry may begin no longer refuse it, and the changes they stand for are classified by currency, the use check and the proofs at their durable boundaries. Only the pending mutation's own request proceeds: any other call is refused by the same-request check that follows (§5.8) |
...
| Phase-entry continuation mode (P2-IMPL-BLOCKER-006) | the same order, with one branch: once the slot's pending review-v1 Phase-entry planning mutation holds a Run with a canonical generation 1, the mutable new-entry admission facts (the Roadmap's and the Phase's lifecycle, the settled-lifecycle check, the dependencies, the explicit entry's current startability) no longer refuse that request; every static and structural check still runs, a call that is not that request is still refused at step 5, and what changed is classified by §13, the use check, §15.6, C-2(Kp) and the committed planning proof. A fresh or pre-generation-1 call keeps the live admission rules; legacy Phase entry and Roadmap creation are unchanged | §5.8, §5.6, §13, §21.1 rows 23 and 25, §28 T |
...
- `review` given → validated before the lock, before any read of Project state: `type(review) is PlanningReview`; `review.contract == "review-v1-planning-v1"` exactly; `callable(review.reviewer)`; `reviewer_identity` and `reviewer_version` are non-empty `str`, equal to their own `strip()`, contain no character `str.splitlines` breaks on, and are canonical Review text — for a Review record holding the value, `serialize.canonical_bytes` succeeds and `serialize.canonical_roundtrips` holds. Anything else → `ValidationError` code `review_contract_invalid`; nothing is read or written.
...
Representability belongs to this gate and not to §5.6, because these two values are not diagnostics: generation 1 makes them durable in the TaskInput and in the accepted descriptor, and `task_input_digest` is taken over their canonical bytes (§10.2). They are not in the request identity I, so §5.6 cannot reach them, and not in the Candidate, so §7.6 cannot either; without this rule the first encode is `planning.accepted_descriptor`, after the lock and after `_open`. Both serializer calls are required: `canonical_roundtrips` does not itself encode and holds for a lone surrogate. The check stays a pure function of the caller's object — it reads no Project state, writes nothing, and uses no serializer but the P1 one — and it is the `review` argument's own validity, so its refusal is `review_contract_invalid`, never the `review_candidate_unrepresentable` of §5.6 and §7.6.
...
2  the live checks that precede the request identity, unchanged:
     ...
     Phase entry:      the structure precheck; the Phase, Roadmap and lifecycle states; the dependencies
                       (in continuation mode the mutable ones of these do not refuse, §5.8)
...
6  the remaining live acceptance checks, unchanged (Phase entry: already expanded, at least one normal
   ...
   none), under the live interrupted-entry rule below and, in continuation mode, §5.8
...
**Which checks of steps 2 and 6 refuse** (P2-IMPL-BLOCKER-006). One branch decides it, and only for Phase entry:

- **A — a fresh review-v1 Phase entry, or one whose pending planning mutation holds no Run with a canonical generation 1.** Every check of steps 2 and 6 runs and refuses as today: the structure precheck, the Phase, Roadmap and lifecycle states, the settled-lifecycle check, the dependencies, the at-least-one-normal-Work and reserved-key checks, and the explicit entry's startability (`_require_startable_entry`, `roadmap.py:811`) — with the two skips of the interrupted-entry rule above wherever a pending mutation of this very request exists.
- **B — a review-v1 Phase entry in continuation mode** (§5.8: the slot's pending review-v1 Phase-entry planning mutation holds a Run with a canonical generation 1). Every static and structural check of A runs and refuses exactly as in A. The mutable facts that decide whether a *new* entry may begin — the Roadmap's and the Phase's lifecycle, the settled-lifecycle check, the dependencies and the explicit entry's current startability — do not refuse that request, and what they stand for is classified by the frozen currency, use-check and proof layers (§5.8).
- **C — a legacy invocation.** Unchanged: it never enters continuation mode, and the live interrupted-entry rule keeps its live meaning there.
- **D — a review-v1 call with no pending planning mutation for the slot.** Unchanged (P2-READY-003): every live acceptance check first, canonical recovery discovery afterwards (step 7). Continuation mode cannot apply, because it is read from a pending planning mutation.

Roadmap creation has no such branch: step 6 has no acceptance check for it, and its step 2 — the payload checks and the structure precheck — runs in every branch.
...
### 5.8 Phase-entry continuation mode

**Frozen** (P2-IMPL-BLOCKER-006). The live Phase-entry checks answer one question — *may a Phase entry begin here now?* — and a review-v1 Phase entry asks a second one on every later call: *may the Phase entry this Project already began be carried to its end?* Deciding the second with the facts that answer the first stops a Run that is already canonical, and two valid Workline operations can then wait for each other: a disjoint-scope Roadmap hold applies its event and is held by the publication barrier until the pending planning operation publishes Km (§18.7, §21.1 row 25), while the planning operation's own retry is refused `roadmap_held` by the working tree that hold just wrote. Neither can finish. This section separates the two questions.

**The boundary.** It is read from canonical state, and from nothing a caller supplies:

- the slot — this Phase — holds a pending planning mutation of the review-v1 Phase-entry operation: a `phase-entry` record carrying the marker pair of §5.2, with or without `recovery_of_review_run_id` (`_pending_phase_entry`, `roadmap.py:787`, with the marker reading of §5.3); and
- the Run that mutation holds — its reservation under the Run key (§6.2, §6.3), or, for a recovery planning mutation, the Run its `recovery_binding` note names (§12.5) — has a **canonical generation 1**: generation 1 committed at HEAD and reading back under the P1 reader, with its candidate snapshot and task input (§11.6, §12.3).

Then the call is in **Phase-entry continuation mode**. A pending mutation that holds no Run, a Run whose generation 1 is recorded or applied but not committed and readable, and a generation mutation merely begun are all outside it: a Review Run that was never accepted canonically has authorized nothing, so the call is an ordinary new entry and §5.6 branch A decides it. Reading the boundary reserves nothing, writes nothing and evaluates no caller value, so it changes the order of no check: the mutable admission facts stay where §5.6 puts them, and only their refusal is conditional.

**What it changes.** In continuation mode these facts no longer refuse the pending mutation's own request:

- the Roadmap's lifecycle (`_require_active_roadmap`, `roadmap.py:768`, `roadmap.py:313-317`: `roadmap_held`, or `spec_violation` for a cancelled or achieved Roadmap) — its first refusal, that the Phase's Roadmap resolves at all (`roadmap.py:311-312`), is a static fact and still refuses;
- the Phase's lifecycle and state (`SpecViolation` for complete, held, cancelled or plan-excluded, `roadmap.py:769-773`);
- the settled-lifecycle check of the Phase and its Roadmap (`_require_settled_lifecycle`, `roadmap.py:778`);
- the Phase's dependencies (`phase_blocked`, `roadmap.py:779-784`);
- the explicit entry's current startability (`_require_startable_entry`, `roadmap.py:811`);
- `phase_already_expanded` (`roadmap.py:793-805`) and the unique-entry check (`roadmap.py:812-817`), which the live interrupted-entry rule already skips for a pending mutation of this request (§5.6).

**Nothing is ignored.** Each of those facts is a state of the Project, and the frozen machinery classifies a change of it at the durable boundary the flow has reached — never as an entry refusal, and never twice:

| where the flow is | who decides a changed fact | outcome |
| --- | --- | --- |
| before a Receipt (latest generation 1 or 2) | currency, §13 items 1–4, at the reviewer launch and before generation mutations 2 and 3 | stale → terminal `stale`, nothing written (§13.2); a Candidate mismatch → `ReconcileRequired` (`review_candidate_mismatch`) |
| generation 3 sealed, no registration stage recorded | the use check (§17 item 7) | stale → generation 4 and the Supersession, then terminal `stale` (§13.3) |
| the registration began, Kp not made | the pre-Kp currency proof (§15.6) | `ReconcileRequired` (`review_registration_currency_changed`, `review_candidate_mismatch`, or `review_registration_base_moved` by the base rules); no Kp, no Consumption, nothing pushed (§13.4) |
| Kp made | C-2(Kp), on Kp's exact parent P (§15.3 P12) | `ReconcileRequired` (`review_persisted_proof_failed`); no Consumption; the barrier holds (§15.5) |
| a push of a history holding Kp | the committed planning proof (§18.8 CP7) | the publication barrier holds (§18.7) |

**What still runs.** Continuation mode is not "skip validation". Every check that proves the call is safely the same operation runs in its frozen place:

- the argument, platform and Git version checks (§5.1, §5.4, §5.7);
- the structure precheck and the existence of the named Phase (`roadmap.py:763-766`) and of its Roadmap (`roadmap.py:311-312`): a Project whose canonical structure cannot be read, or whose named entities do not resolve, fails closed, here as everywhere;
- the request identity (§6.2) and the canonical-input preflight (§5.6);
- the same-request check and the marker check (§5.3);
- the design's static shape: at least one normal Work (`roadmap.py:806-807`), the reserved Work keys (`roadmap.py:808-810`), and every other malformed-design refusal of the live entry;
- everything the resumed mutation runs for itself: the replay's projected refusals (`refuse_invalid_work_writes`), the operation binding (§11.5), the currency evaluations (§13), the use check (§17), the dirty-overlap and display base checks, the proofs and the publication barrier.

**A call that is not this request.** The boundary is read from the slot's pending records, so another *review-v1* call for the same Phase also passes the mutable admission facts — and is then refused where the contract already refuses it, at §5.6 step 5: `reconcile_required` (live, reason `None`) for a different design, `review_marker_mismatch` where the markers do not match (§5.3). Nothing is written, and the pending record is untouched. A legacy invocation is §5.6 branch C and never a continuation: the live admission checks refuse it exactly as today, and one that passes them meets the marker mismatch at step 5, as before. So the only call continuation mode lets past step 5 is the exact request the pending planning mutation records.

**Uncommitted state is never currency.** An operation the publication barrier holds back stays pending with its domain effects applied in the working tree (§18.7). Those bytes are no committed declared-base change: every currency evaluation and every proof reads the committed view of an exact commit (§13, §15.4), so the continuation is decided by what is committed, and may finish C-2(Kp), the Consumption, Km, C-2(Km) and the publication on that basis. Once Km is published the held operation's own retry commits and publishes its change (§21.1 row 25). A *committed* change of the same fact reaches the table above unchanged.

This is the one behaviour the branch changes on purpose: a lifecycle change another operation has decided, or applied to the working tree, but not committed no longer stops the continuation. What the continuation registers was authorized on committed state, and the other operation's decision takes effect when it commits, so the Project ends holding both facts — the registered Works, and the new lifecycle — exactly as it would had the two operations run in the other order. For a *new* entry nothing of this applies: the settled-lifecycle check and every other live refusal still decide it (§5.6 branch A).

**No new authority.** Continuation mode authorizes nothing, writes nothing, and reads no Review record as lifecycle truth: it decides only which of the two questions the live admission checks are being asked. Review stays a subordinate gate (§22), the Roadmap operation stays the owner of the entry and of this branch, and `state.py` and every lifecycle derivation are untouched. It is no recovery rule either: with no pending planning mutation the order of §5.6 and canonical recovery discovery are exactly as P2-READY-003 froze them.

**Roadmap creation** has no mutable admission check of this kind (§5.6 step 6: it has no remaining live acceptance checks), so nothing here applies to it. **Legacy Phase entry** is unchanged: a legacy invocation never enters continuation mode, and the live interrupted-entry and abandonment rules keep their live meaning there (§12, §26.2).
...
| C | ambiguous, C startable but not among the ties | `StopError` code `review_entry_ambiguous` (P2-IMPL-BLOCKER-005) |
...
**Which refusal an explicit entry gets** (P2-IMPL-BLOCKER-005). The canonical selection decides, never the caller's Work:

- the selection is **ambiguous** (`len(preferred) > 1`): every explicit entry is refused with `review_entry_ambiguous`, whether the caller named one of the tied Works or another startable Work outside the ties. The committed basis does not say which Work comes first, so no explicit entry can be the canonical one, and the refusal names that;
- the selection is **unique A**: an explicit B ≠ A is refused with `review_entry_not_canonical`, which is reserved for exactly this case — a canonical first Work exists and the caller named another.

A design that must start at a particular Work says so by planning it (`planned_next`), which makes the selection unique; naming an entry never overrides the committed basis (Frozen P1 R9 §4).
...
1. §5.6 steps 1-6: the argument, platform and Git version checks; the live checks up to the request identity;
   ...
   checks — the live checks of step 2 and those of step 6 under the live interrupted-entry rule and, for a
   Phase entry whose Run has a canonical generation 1, under the continuation branch of §5.8, which
   suspends the mutable admission facts of both steps and no others
...
The working-tree round trip (§15.1 step 2) rebuilds no Candidate, but its comparison carries the R9 selection, so the same precedence binds it (P2-IMPL-BLOCKER-003): it compares only while the declared base on HEAD's committed view is the Candidate's, and defers a difference to the pre-Kp currency proof of step 3, which classifies it by item 3. With the declared base equal it compares as before, and a genuine round-trip difference there is still `review_roundtrip_mismatch`.

A change that another operation has applied to the working tree but not committed is no declared-base change at all: every evaluation above reads the committed view of its base commit B (§15.4), so an operation held back by the publication barrier (§18.7) leaves the currency of a pending planning operation exactly as it was (§5.8).
...
**Which refusal a configured driver gives** (P2-IMPL-BLOCKER-001). Part 1's driver condition and §14.5 layer 4 read the same configuration key, so a repository that defines a filter driver named `unset` fails both. The order decides the code, and it is this order everywhere the two run together (§14.4 step 5, §11.6, §15.1): part 1 is evaluated first, so a review-v1 call in such a repository stops with `StopError` code `review_git_transform`, at the freeze abandoning the planning mutation and afterwards leaving it pending, with nothing written either way. `review_checkout_unsafe` is what evaluating the capability of §14.5 on its own gives for the same repository — the refusal §28 J tests directly, and the one a call meets where only §14.5's condition fails (a driver named `unspecified` is part 1's alone, and every other layer failure is §14.5's alone). Both are fail-closed and neither writes; nothing depends on which of the two a caller sees.
...
Layer 4's driver condition is refused in the same spirit, and deliberately (P2-IMPL-BLOCKER-001): a repository whose effective configuration defines a filter driver named `unset` is refused even where the canonical rule's `-filter` keeps Git from ever selecting it, and even where a fresh clone made under that configuration keeps every Review record byte for byte (**Measured**, above). Layer 4 exists because the printed word `unset` cannot be told from a literal value, so the repository must hold no driver that word could select. A configuration that defines one is outside the single form P2 v1 proves positively; P2 refuses what it cannot prove, and never argues from a byte probe that a configuration it does not support is safe enough. Removing the driver, or renaming it, restores the capability with nothing else changed.
...
2  working-tree round trip, under the declared-base precedence guard below: the declared base on HEAD's
   committed view is compared with the Candidate's first. Equal -> the round trip compares
   ProjectView.load(store) -> normalize_persisted == reviewed, the registered entities and relations read
   from the working tree and the R9 selection from HEAD's committed basis (§7.7); mismatch ->
   ValidationError review_roundtrip_mismatch (P1 adapter code), pending. Different -> the round trip
   classifies nothing, and step 3 classifies the difference (§13 item 3)
...
10 live structure postcheck (_stop_on_structure "postcheck"); the result's entry Work is the Candidate's
   canonical_first_work, never recomputed from the working tree (below, §7.7, §19.3); complete()
```
...
**The declared-base precedence guard** (P2-IMPL-BLOCKER-003). Step 2's comparison carries the R9 selection, which HEAD's committed basis gives (§7.7). A committed declared-base change that also changes that selection would therefore be reported as a round-trip mismatch before step 3 could classify it — against §13, which classifies a declared-base difference once and never also as an R9 or Candidate mismatch, and against §21.1 row 20 and §28 P-E, which name the pre-Kp currency proof as its owner. The guard is that precedence and no new currency check: it computes the declared base on HEAD's committed view exactly as §13 item 3 does, with the same function on the same committed view, and where it differs from the Candidate's it leaves the classification to step 3 — `review_registration_currency_changed` for a stale difference, `review_candidate_mismatch` for a Candidate mismatch, or the base reasons of §15.6. Either way nothing is committed, no Kp is recorded or made and the planning mutation stays pending with its registration in the working tree. Where the declared base is equal, step 2 is exactly the check it was: the round trip still proves that the working tree holds what Review authorized, and a difference there is still `review_roundtrip_mismatch`, the P1 adapter code.

**The entry Work after the registration** (P2-IMPL-BLOCKER-006). The legacy expansion recomputes the entry from the working tree once it has finalized: `ProjectView.load(store)`, `startable_works`, and `postcheck_failed` for an explicit entry that is not startable there (`roadmap.py:1023-1032`). On the review-v1 path that recomputation decides nothing. The R9 selection is evaluated on the committed basis only (§7.7, §22 item 4); it was proven there by the pre-Kp proof, by P8/P9 and by CP6; and the result's `entry_work_id` is the Candidate's `canonical_first_work` (§19.3). So no working-tree fact — a person's uncommitted event, or another operation's applied but uncommitted effect — refuses a registration that the proofs have passed (§5.8). The live structure postcheck still runs (step 10), and legacy keeps the recomputation exactly as it is.
...
**P12 reads P, and only P** (P2-IMPL-BLOCKER-002). P12's currency is evaluated on Kp's exact parent — the commit the registration was made on — never on HEAD and never on a descendant of Kp. A fact committed after Kp exists lies in a descendant and can never be what P12 sees: the pre-Kp currency proof (§15.6) is what stands between the registration and such a fact before Kp exists, and afterwards the base rules of §15.2 and §15.6 decide a moved HEAD. P12 is defence in depth — it re-proves on P what the pre-Kp proof proved on P, so that a Kp reached through a bypassed, patched or crashed pre-Kp step is still refused before any Consumption is written. A test of P12 therefore builds a P that itself holds the mismatching committed pre-state; a later commit on top of an honest Kp is a different scenario with a different owner (§28 D, §28 P-E).
...
**Cost.** The proof runs for each registered Run in the published history, at each push that would write and at the freeze or recovery setup of a review-v1 call with a push destination. It is deterministic and bounded by those Runs' records, two committed views and one materialized projection base each.

A Project that never ran a review-v1 planning invocation has no registered Run and runs no part of this proof. It pays the fast path of §18.7 — one path-limited history read **per distinct commit ID the barrier evaluates**, which every Git the live baseline runs on answers, with no P2 minimum (P2-IMPL-BLOCKER-004). A normal legacy push evaluates two distinct commits, because §18.7 checks different commits at its check points: HEAD before the Git stage holding the push is recorded, and the commit that stage makes, at its classification and again right before the push. The two evaluations of the commit made reuse the first result, as §18.7 permits for one commit ID within a process, so such a push pays two reads and no more. What the rule protects is that a legacy-only history takes the cheap path only: no registered-Run discovery, no committed planning proof, no Review evaluation and no Git minimum — not that the operation runs exactly one Git command.
...
In every row the next same-request run reaches the row's own rule. For a review-v1 Phase entry whose Run has a canonical generation 1, the mutable new-entry admission facts no longer refuse it (§5.8), so a lifecycle change of the Phase or its Roadmap that another operation has applied or committed decides the outcome at the durable boundary the row names — currency, the use check, the pre-Kp proof, C-2(Kp) or the committed planning proof — and never as an entry refusal before the row is reached (P2-IMPL-BLOCKER-006).
...
| 19 | registration fully applied, round trip / pre-Kp proof not run, Kp not recorded | resume; the round trip runs under the declared-base precedence guard (§15.1 step 2: with the declared base different it classifies nothing and the proof below does); the pre-Kp currency proof on P = HEAD (§15.6), its physical projection item included: pass → Kp recorded with `base_head` = P and `base_exact`, and made; failure → `ReconcileRequired` with its §15.6 reason, nothing committed, no Consumption, pending |
...
| 23 | Kp made, `commit_id` saved, C-2(Kp) not run | the proof runs over the recorded Kp (P1–P12); the barrier holds for every history holding Kp until the committed planning proof proves Kp, its Consumption and Km for the commit being published (§18.7). The same-request run reaches that proof even while another operation waits at the barrier with its domain effects applied to the working tree: after canonical generation 1 the Phase entry is admitted as a continuation (§5.8), and uncommitted effects are no currency (§13) |
...
| 25 | another Workline operation attempts a push while Kp is unproven (rows 22–24, 26–28) | `review_publication_barrier` (§18.7): its current-combined stage is not recorded (the mutation stays pending with its domain effects applied), or its recorded push is not classified unapplied / not applied; nothing is pushed and the destination never receives Kp; it continues once Km exists, the committed planning proof passes and Km is published (its commit then fast-forwards over Km, or is classified already published) — or, when Kp can no longer be proven, after a person reconciles the branch history. **The path to Km stays open** (P2-IMPL-BLOCKER-006): the domain effects that operation applied are uncommitted, so no currency evaluation and no proof reads them (§13, §15.4), and a same-request review-v1 Phase entry whose Run has a canonical generation 1 is admitted as a continuation whatever lifecycle those effects show (§5.8). So the planning operation always reaches C-2(Kp), the Consumption, Km and its publication, and the held operation continues afterwards; the two never wait for each other. The same holds when that operation's change is committed instead: it is then classified by currency, the use check, the pre-Kp proof, P12 or CP7, at the boundary the flow has reached |
...
Variants at any row: a different request or design → `reconcile_required` (live); a legacy invocation against a review-v1 record, or the reverse → `reconcile_required` (`review_marker_mismatch`, §5.3); a pending generation mutation bound to another planning mutation → reconcile (`review_generation_owner_conflict`, §11.8); structure changed by an independent operation while pending → the live projected refusal (`postcheck_failed`) before replay; the parent Roadmap held while a Phase entry is pending → before the Run's canonical generation 1, the live Phase-entry precheck STOP (`roadmap_held`), continued once the Roadmap is resumed; after canonical generation 1, the entry is admitted as a continuation (§5.8) and the hold decides nothing until it is committed — then currency, the use check or the pre-Kp currency proof decides it (a declared-base difference is `ReconcileRequired` once the registration began, §13.4), and after Kp is made P12 reads P alone (§15.3).
...
12. Phase-entry continuation mode (§5.8) gives Review no lifecycle authority and no authorization power. It reads the slot's pending planning records and one committed gate to tell a new entry from the continuation of one already accepted, writes nothing, and changes no Phase, Roadmap or Work state. A lifecycle change it does not treat as an entry refusal is not ignored: currency, the use check and the proofs classify it, each on committed state (§13, §17, §15.6, §15.3, §18.8). Whether a Phase entry may *begin* still depends on the live lifecycle facts alone, and a legacy Phase entry is untouched.
...
- a `ReconcileRequired` that live code raises on the review-v1 path keeps `reason` `None` and its live message, as in legacy — the same-request mismatch, a scope overlap in `MutationController.open`, a `reserve_id` kind mismatch, the live settled-lifecycle check of the entry (`_require_settled_lifecycle`, `roadmap.py:277-307`), which a continuation does not run (§5.8), the live branch binding, a live replay or push-classification mismatch;
...
| `phase_already_expanded`, `ambiguous_startable_candidates`, `phase_blocked`, `roadmap_held`, `spec_violation` | live codes, meaning unchanged | §5.6 steps 2 and 6; §7.7. For a Phase entry in continuation mode the mutable ones of them do not refuse that request (§5.8); every other call keeps them exactly | before `_open`: nothing begun; at the freeze: abandoned |
...
| `review_roundtrip_mismatch` | `ValidationError` code (P1) | §15.1 step 2, and only with the declared base on HEAD's committed view equal to the Candidate's: a declared-base difference is deferred to the pre-Kp currency proof (§13, P2-IMPL-BLOCKER-003) | pending |
...
- Phase-entry continuation mode (§5.8): once the slot's pending review-v1 Phase-entry planning mutation holds a Run with a canonical generation 1, the mutable new-entry admission facts no longer refuse that request, every static and structural check still runs, a call that is not that request is still refused at the same-request check, and a changed fact is classified by currency, the use check and the proofs. The Skill states which live checks that covers, that a fresh or pre-generation-1 call keeps them all, and that legacy Phase entry is unchanged;
...
| `src/workline/roadmap_review.py` (new, Roadmap-owned) | the review-v1 flow: the canonical-input preflight (§5.6), a pure function over the request identity that uses only `validate_condition` and the P1 serializer; the review-v1 entry order with the live interrupted-entry rule (§5.6), the Phase-entry continuation boundary read from the pending planning records and the Run's committed generation 1 (§5.8), and the Git version gate of `P2_REVIEW_GIT_MIN` (§5.7); recovery orchestration (discovery call before `_open`, the recovery planning mutation, its binding and continuation, the pre-freeze resume setup, §12.9), the pre-generation-1 abandonment rule (§12), freeze with the committed basis and the working-tree compatibility check (§7.4, §7.6, §7.7), generation mutations 1–4, reviewer call, currency on committed views in the order of §13, use check and `use_check_head`, registration hand-off, the working-tree round trip under the declared-base precedence guard (§15.1 step 2), pre-Kp currency proof (and its `refuse_recorded` hook), Kp / Km, proofs, Consumption, publication, terminal outcomes; W (`CanonicalPlanningWriterInput`, §7.8), the one Candidate-to-writer-input function every consumer takes; both adapters, with the expected physical projection (`project_expected`, §15.7), its comparison and record check, and the display base check; `ReviewedPlanningResult`; every `ReconcileRequired` it raises carries its §25.1 reason | §5.6, §5.7, §5.8, §7, §11–§21, §25.1 | A–T |
...
| `src/workline/roadmap.py` | `review=` keyword; on the review-v1 path, the canonical-input preflight called right after the request identity is computed and before the same-request check and `_open` (§5.6); marker check at the entry (§5.3); the continuation branch of the live Phase-entry admission checks, decided from the slot's pending planning records and the Run's committed generation 1, with legacy Phase entry and Roadmap creation unchanged (§5.8); the registration of `_create_roadmap` / `_expand_phase` separated from the legacy `_finalize` without changing legacy behaviour; the Roadmap-file rendering of `_create_roadmap` and the stage-input derivation of `_create_roadmap` / `_expand_phase` extracted as pure functions the legacy path itself calls, with no behaviour change (§7.8, §20); review-v1 branch, whose registration takes W | §5, §5.8, §7.8, §15.1, §15.7 | A, B, I, M, N, O, T |
...
- Every R9 case of §7.7, each row of its table its own case: an explicit entry that is startable but outside the ties of an ambiguous selection → `review_entry_ambiguous`, like an explicit entry among the ties (P2-IMPL-BLOCKER-005); `review_entry_not_canonical` only where the selection is unique and the caller named another Work; and no case where an explicit entry decides the selection.
- The declared base changes before the seal → stale, nothing written (§13.2).
- *P2-CONTRACT-003, the recomputed factors.* Each of authority text, the Policy constant (patched) and the loader identity (a package file changed) changed at four moments: after the review and before the registration (→ generation 4, stale); after the first registration stage and before Kp is recorded (→ pre-Kp currency proof `ReconcileRequired`, no Kp, no Consumption, no push, no generation 4); after the Kp stage is recorded and before it is made (→ the `refuse_recorded` hook's pre-replay currency proof refuses, Kp not made); after Kp is made (→ each is recomputed now, so C-2(Kp) P12 refuses, no Consumption, the barrier holds).
- *P2-CONTRACT-003, the declared base* (P2-IMPL-BLOCKER-002). The same moments with a declared-base fact the kind actually reads (§7.4), changed by a committed disjoint-scope operation in a crash window:
  - PhaseEntryDesign: a hold of the Phase's Roadmap (the declared base's `roadmap: {id, lifecycle}`), or a predecessor's state (`phase_dependencies`). Moments 1 and 2 give the outcomes above, and the retry reaches them because continuation mode admits it (§5.8, §28 T). At moment 3 that commit has moved HEAD off the recorded `base_head`, so the `refuse_recorded` hook classifies the recorded Kp a mismatch and refuses with `review_registration_base_moved` before any currency is evaluated (§15.2), and Kp is not made.
  - RoadmapPlan: the lifecycle or state of a Phase the plan's relations name (`existing_phases`) — a hold of that Phase itself. A hold of that Phase's *Roadmap* changes no RoadmapPlan declared base: `phase_lifecycle` derives from the Phase's own events and `phase_state` from that lifecycle and the Phase's own Works (`state.py:209-210`, `state.py:289-301`), while a `roadmap_held` event reaches only `derive_roadmap_lifecycle` (`state.py:92-104` reads the Phase's four events; `state.py:106` reads the Roadmap's). No test claims otherwise.
  - Moment 4 is no runtime window for this factor: a fact committed after Kp exists lies in a descendant of Kp and never in P, the only commit P12 reads (§15.3). What the window really gives is tested instead — with a push destination the disjoint-scope operation's Git stage is held by the barrier while Kp is unproven (§18.7, §21.1 row 25), the planning operation continues under §5.8, proves Kp and publishes Km, and that operation then commits and publishes its own change; remote-less, it commits on top of Kp and the registration completes through C-2(Kp) on P, the Consumption and Km.
- *P2-CONTRACT-003, the P12 and CP7 fixtures.* P12 is exercised directly, as the defence-in-depth proof it is (§15.3): a fixture puts the changed declared-base fact into P itself and bypasses the pre-Kp proof, and C-2(Kp) refuses naming P12, with no Consumption and the barrier holding. CP7 is exercised the same way over a committed history whose P holds the changed fact (§28 P-E, §28 K).
...
- **I — Hostile global and system configuration, no driver `unset`.** With `core.autocrlf=true`, a global attributes file asking `*.yaml eol=crlf`; `*.yaml filter=unset` with no filter driver named `unset` configured; `*.yaml ident`; `*.yaml working-tree-encoding=UTF-16`; `*.yaml text eol=crlf`; or the legacy `*.yaml crlf`, `-crlf` or `crlf=input`; then all of them at once with `core.autocrlf=false` and `core.eol=crlf`, given as global and, separately, as system configuration: the committed canonical rule outranks every one of them — all four layers pass, the capability holds, both evaluations print form L, and a fresh clone keeps every Review record byte for byte.
- **I2 — The same configuration with a driver `unset` configured** (P2-IMPL-BLOCKER-001). Each of I's configurations, and all of them at once, with a filter driver named `unset` whose smudge changes bytes defined beside them, as global and, separately, as system configuration: the capability evaluated on its own fails at layer 4 → `review_checkout_unsafe`, and a review-v1 call in that repository stops at the freeze with `review_git_transform`, which part 1 of the Git persistence preflight raises first for the same configuration key (§14.3) — before any Review record, the planning mutation abandoned, nothing written. The fresh-clone byte probe of the same fixture shows every Review record kept byte for byte, because the canonical rule's `-filter` selects no driver: that is why the refusal is a supported-configuration refusal and not a byte failure. Removing the driver makes the same fixture pass (item I). No test takes the byte probe, or the printed form L, as proof of the capability.
- **J — Current `info/attributes` override.** Layers 1 to 3 pass. `.git/info/attributes` asking `.workline/review/** eol=crlf` → the effective evaluation prints `eol: crlf` → `review_checkout_unsafe`. The same file asking `.workline/review/** filter=unset`, with a filter driver `unset` configured → the effective evaluation prints form L and layer 4 fails on the driver → `review_checkout_unsafe` where the capability is evaluated on its own, and `review_git_transform` at the freeze, part 1 of the preflight refusing the same key first (§14.3); with no driver `unset`, the literal selects nothing and the capability holds.
- **K — Literal filter exploit.** A driver named `unset` whose smudge changes bytes, configured globally. The committed literal `filter=unset` fixture, whose evaluations print form L, is refused by layer 1. A fresh clone of that fixture made under this configuration changes the Review records' bytes, which is why it is refused; the canonical rule under the same configuration keeps them, and is refused all the same, by layer 4 (item I2).
...
- A legacy-only Project's push performs one path-limited fast-path history read per distinct commit ID the barrier evaluates, and no proof (P2-IMPL-BLOCKER-004). A normal legacy push evaluates two commits — HEAD before its Git stage is recorded, and the commit that stage makes, at classification and again right before the push — so the test asserts exactly two reads, the repeated evaluation of the commit made reusing the first result within the process (§18.7, §18.8). Registered-Run discovery, the committed planning proof, every Review evaluation and the Git-version read stay at zero, and the test asserts each of them.
...
- **E — Declared base first.** A committed lifecycle change of a declared-base fact after generation 1, 2 or 3 that also changes the R9 selection is classified by the declared base, never as an R9 or Candidate mismatch. Each point is exercised where it is reachable, with a fact that can change there (P2-IMPL-BLOCKER-002):
  - stale before a Receipt — a predecessor Work completed, as a person's commit: START declares every ledger, so no Workline START can commit one while a planning mutation is pending (§13.5, scope overlap);
  - generation 4 at the use check — the same fact, committed after the seal;
  - `review_registration_currency_changed` at the pre-Kp proof — the same fact, committed after the first registration stage while HEAD is still the registration's base;
  - the working-tree round trip of §15.1 step 2 classifies none of them: with the declared base different it defers to the pre-Kp proof, so `review_roundtrip_mismatch` is never the outcome of a declared-base change (P2-IMPL-BLOCKER-003), and with the declared base equal it still refuses a genuine round-trip difference;
  - P12 in C-2(Kp) — a fixture whose P itself holds the changed fact, with the pre-Kp proof bypassed, because a fact committed after Kp is never in P (§15.3);
  - CP7 in the committed planning proof — a committed history whose P holds the changed fact.

  "The Phase held" is such a fact for a Phase entry, and it arrives as a person's commit: a Workline `phase-hold` of the entering Phase declares that Phase (`roadmap.py:1157-1163`, `_open` entities `roadmap.py:819-820`) and overlaps the pending planning mutation's write scope, so `MutationController.open` refuses it (`reconcile_required`, reason `None`) while the entry is pending. After canonical generation 1 the entry itself is admitted as a continuation, and the committed hold is classified at the point the flow has reached, never as an entry refusal (§5.8, §28 T).
...
- **D — Own applied stages.** A pending review-v1 Phase-entry mutation of this very request, with its own registration stages applied, is resumed under the live interrupted-entry rule: it is never refused as `phase_already_expanded`, and the unique-entry check is skipped for it. Such a mutation's Run has a canonical generation 1, so the explicit entry's startability is not re-checked either (§5.8, §28 T); a pending mutation whose Run has none keeps that check (§28 T item E).
...
**T. Phase-entry continuation mode (P2-IMPL-BLOCKER-006)**

- **A — The deadlock regression.** One test, end to end, with a push destination and no manual Git at any point:
  1. a review-v1 Phase entry runs to a made Kp whose commit ID is durably saved, with C-2(Kp) not yet run (a crash at the proof);
  2. `hold_roadmap` on that Phase's Roadmap — a disjoint-scope operation, the event log only (`roadmap.py:118-140`) — applies its event and is stopped by `review_publication_barrier` before its Git stage is recorded; its mutation stays pending with the event in the working tree, and the destination receives nothing of Kp;
  3. the same-request Phase-entry retry is **not** stopped by `roadmap_held`, nor by the Phase's state, the settled-lifecycle check, the dependencies or the entry's startability;
  4. it continues as the same planning mutation: C-2(Kp), the Consumption, Km, C-2(Km), and the publication of exactly Km;
  5. the pending Roadmap hold, retried, then completes: its commit is made and pushed under the unchanged lifecycle rules, on top of Km;
  6. no manual Git, no deleted mutation record, no reset, rebase or amend, and no rewritten lifecycle event at any step. Both operations end through Workline, and the planning operation ends `registered`.
- **B — The same without a destination.** Remote-less, the hold commits at once (no barrier), and the Phase entry continues over the committed hold: its declared base changed, so the pre-Kp currency proof or C-2(Kp) decides it by §13 — never `roadmap_held`.
- **C — The boundary.** A pending review-v1 Phase-entry planning mutation whose Run has no committed generation 1 — interrupted at the freeze, or in generation mutation 1 before its commit, or holding no Run at all — with the Roadmap held: the live `roadmap_held` refusal, nothing written, the pending mutation untouched. With generation 1 committed and reading back: continuation. A recovery planning mutation is covered too, its Run read from the `recovery_binding` note.
- **D — Every suppressed check.** With the Run at canonical generation 1 and the planning mutation pending, each of these is made true in turn and the same-request retry is admitted past the entry checks — no `roadmap_held`, `spec_violation`, `phase_blocked`, settled-lifecycle reconcile or startability refusal: the Roadmap held; the Roadmap cancelled or achieved; the Phase held, cancelled or plan-excluded; a lifecycle decision of the Phase or its Roadmap decided by another pending mutation and not yet applied; an unsatisfied dependency of the Phase; the explicit entry no longer startable. Each change then decides the run at the flow's own boundary (item G) where it is committed, and decides nothing where it is not (item H).
- **E — Before the boundary, every check refuses.** The same changes against a pending mutation before canonical generation 1: each gives its live refusal, unchanged, with nothing written.
- **F — Static checks still refuse in continuation mode.** With the Run at canonical generation 1: a design with no normal Work; a reserved Work key; a caller value the canonical form cannot carry (§5.6); a review-v1 request that is not the pending mutation's (`reconcile_required`, the live refusal); a legacy invocation against the review-v1 record, which is never a continuation and meets the live admission checks first and the marker mismatch after them (`review_marker_mismatch`); and a Project whose canonical structure cannot be read (the live structure refusal) — each stops the call with nothing written and the pending record untouched.
- **G — Committed changes are never swallowed.** With canonical generation 1 and the fact changed *and committed*: before a Receipt → §13 decides (stale, nothing written); after generation 3 with no registration stage → the use check decides (generation 4 and the Supersession when stale); after the registration began and before Kp → §15.6 decides (`review_registration_currency_changed`); after Kp is made → P12 reads P alone, so a change committed on a descendant is not what it sees (§15.3, §28 D, §28 P-E).
- **H — Uncommitted changes are never currency.** The same changes applied only to the working tree by another pending operation whose push the barrier holds: no currency evaluation and no proof reads them, and the run's Candidate, use check, proofs and outcome equal those of a run with no such operation at all.
- **I — Legacy unchanged.** A legacy Phase entry into a Phase whose Roadmap is held is refused exactly as at the baseline, with a pending legacy record and with none, and a legacy interrupted expansion keeps its live rule, including that it is never abandoned.
- **J — Roadmap creation.** A review-v1 Roadmap creation is unaffected: its entry has no such check, and a held Roadmap elsewhere in the Project refuses nothing.
- **K — No lifecycle change.** After item A the Project's end state equals a legacy run's plus exactly the hold's own event: continuation mode wrote nothing and changed no lifecycle fact (§22 item 12).
- **L — The entry Work is the reviewed one.** With the registration proven and an uncommitted working-tree change that would make an explicit entry unstartable — a person's `phase_held` line in the event log, or another operation's applied but uncommitted hold — the run completes, `entry_work_id` is the Candidate's `canonical_first_work`, and no `postcheck_failed` is raised (§15.1, §19.3). A legacy Phase entry in the same situation keeps its live `postcheck_failed`.
...
Implementation blocker repair round 1 search. It covered `always runs`, `roadmap_held`, `Phase held`, `Roadmap hold`, `continuation`, `generation 1`, `same-request`, `interrupted`, `roundtrip_mismatch`, `review_registration_currency_changed`, `one path-limited history read`, `per push`, `review_entry_ambiguous`, `review_entry_not_canonical`, `filter=unset`, `driver named unset`, `capability holds`, `row 25`, `P12` and `CP7`, looking for the superseded statements:

- **"Every other check of steps 2 and 6 always runs": no such statement.** `always runs` occurs only for the add-history read's one spelling (§28 S) and in §33.8's quotation of the sentence this repair replaced. §5.6 now gives four branches (A–D), §5.8 states which checks continuation mode suspends and which still refuse, and §11.8 step 1, §21.1, §25.1, §26.2, §27 and §28 D, P-E, R and T name the same branch.
- **A Phase entry made permanently inadmissible by another operation's applied effects: no such statement.** `roadmap_held` and the held-Phase refusals occur as the live refusals of a fresh or pre-generation-1 call (§5.6, §5.8, §25.1, §28 T C, E, I), and every statement of the post-generation-1 case names the continuation (§5.8, §21.1 rows 23 and 25, §21.3, §28 D, P-E, T).
- **An uncommitted lifecycle change read as currency: no such statement.** §13, §5.8 and §28 T H say the opposite, and every currency and proof point reads the committed view of its base commit (§13, §15.4, §15.6, §15.3, §18.8).
- **A declared-base difference reported as a round-trip mismatch: no such statement.** `roundtrip_mismatch` occurs only with the guard (§15.1 step 2 and its paragraph, §13, §25.1, §28 P-E), always with the declared base equal.
- **P12 or CP7 reading a commit after Kp: no such statement.** §15.3 states that P12 reads P alone, §13.1 gives P as its base, and §28 D and P-E build their P12 and CP7 cases as fixtures whose P holds the changed fact.
- **A working-tree recomputation deciding a proven registration: no such statement.** §15.1's entry-Work paragraph, §7.7 and §22 item 4 keep the R9 selection on the committed basis, and §19.3 takes the result's entry Work from the Candidate; the legacy recomputation stays legacy's (§28 T L).
- **One Git command, or one read, per legacy push: no such statement in a normative section.** §18.8, §2 and §28 K say one path-limited read per distinct commit ID the barrier evaluates, two for a normal legacy push; §18.7's check points and reuse rule are unchanged; and round 2's "one history read per push" survives only in §33.2, where it is marked as corrected here.
- **An explicit entry accepted under an ambiguous selection, or refused as not canonical there: no such statement.** §7.7's table and the paragraph after it give `review_entry_ambiguous` for every explicit entry under an ambiguous selection, and reserve `review_entry_not_canonical` for a unique selection; §28 D names both.
- **The capability holding with a filter driver named `unset` configured: no such statement.** §14.5 layer 4 and its "Unsupported is not unsafe" paragraph refuse it, §21.4 R8 lists it, §28 J I, I2, J and K state the split between the byte probe and the capability, and §34.7 Q7 carries the same caveat. §14.3 says which of the two refusals a call meets first, so the two rules that read the same configuration key name one outcome between them.
...
Implementation blocker repair round 1 ran no new probe of its own. Its six findings were met while implementing this contract against the live baseline, and each was re-checked here against the live code at `b78be1c`, cited where used: `roadmap._enter_phase_locked` (`roadmap.py:762-820`, the order and the exact checks), `_require_active_roadmap` (`roadmap.py:310-317`), `_require_settled_lifecycle` (`roadmap.py:277`), `_require_startable_entry` (`roadmap.py:833`), `_require_unique_entry` (`roadmap.py:897`), `_pending_phase_entry` / `_require_resumable` (`roadmap.py:684-695`), `OPERATION_LEDGERS` and the declared entities of each operation (`roadmap.py:118-140`, `roadmap.py:819-820`, `roadmap.py:1050`, `roadmap.py:1157-1163`), `MutationController.open`'s scope overlap, `state.phase_lifecycle` / `phase_state` (`state.py:209-210`, `state.py:289-301`) and `gitops.finalize` with the barrier's check points (§18.7). The deadlock of P2-IMPL-BLOCKER-006 was reproduced end to end on an unpushed implementation of this contract — a Roadmap hold applying its event and stopping at `review_publication_barrier`, the same-request Phase entry then stopping at `roadmap_held`, `resume_roadmap` refused by the pending hold's scope, and the hold refused again by the barrier — which is a live observation of the contract's own rules, not an authority: no implementation is landed, and nothing in this document rests on one (§32).
...
HUMAN: **None.** HUMAN-1 is resolved (A). Every choice made here, the round-1 to round-5 repairs, both final readiness repair rounds and the implementation blocker repair round 1 included, is an engineering choice bounded by live code and frozen text, and each has its reason stated in its section.
...
Implementation blocker repair
  round 1 contract under repair: de3681c94d9a0c8bf1ac8263513b5793c495a02e
P1 accepted checkpoint:         b78be1ccd778ceb2ecd97b90e2f71744bd1b3bc5
...
P2-IMPL-BLOCKER-001 .. 006:     CLOSED (implementation blocker repair round 1) (§33.8, §34.8)
...
Phase-entry continuation:       after its Run's canonical generation 1, a same-request review-v1 Phase entry is
                                a continuation: the mutable new-entry admission facts no longer refuse it, the
                                static and structural checks all run, and currency, the use check and the proofs
                                classify what changed; legacy and Roadmap creation unchanged (§5.8)
Round-trip precedence:          the declared base on HEAD's committed view is compared before the R9-carrying
                                working-tree round trip; a difference belongs to the pre-Kp currency proof
                                (§15.1 step 2, §13)
...
Checkout capability:            the canonical rule as the last attribute rule of HEAD's raw root .gitattributes;
                                ...
                                no filter driver named unset — refused even where the canonical rule would keep
                                Git from selecting such a driver, because unsupported is not proven (§14.5)
...
**After the implementation attempt** (implementation blocker repair round 1, §33.8). The verdict above stands for `6004669`, the contract as it then was, and is kept as the historical fact it is. A P2 implementation attempt on the verdict-recording checkpoint `de3681c` then met six contradictions in that contract — one of them a deadlock between two valid Workline operations — and stopped before its first push, with nothing landed. This repair closes all six, and so changes contract semantics after that verdict was given: the verdict does not extend to the repaired text. The sequence is exactly this, and none of its steps is rewritten by a later one:
...
- The legacy default is unchanged: a legacy invocation runs no discovery and no proof beyond the barrier, and a Project without review-v1 registrations pays only the fast path. Round 2 wrote that as "one history read per push"; the implementation blocker repair corrects the count to one path-limited read per distinct commit ID the barrier evaluates — two for a normal legacy push (§18.8, §28 K, §33.8).
...
### 33.8 Implementation blocker repair round 1 (on `de3681c`)

The P2 implementation of this contract, carried out on `de3681c` and never pushed, met six contradictions this contract could not answer mechanically, and stopped before its first push. Each was confirmed against the live code at `b78be1c` and against the frozen P1 texts before it was repaired here (§31). Only these six are repaired; P2-CONTRACT-001 to -008, P2-READY-001 to -004 and P2-READY-029 stay closed and unchanged (§34.8). This is not a readiness repair round: the defects were found by implementing the contract, not by reviewing it.

| finding | failure, confirmed against live code and frozen text | repair | sections |
| --- | --- | --- | --- |
| P2-IMPL-BLOCKER-001 (MID) — §28 J-I contradicted §14.5 layer 4 | J-I required "the capability holds" for a hostile configuration that includes a filter driver named `unset`, while layer 4 refuses exactly that configuration (`review_checkout_unsafe`) and §14.3 refuses every review-v1 stage under it. Both cannot hold, and the implementation had to disobey one of them | layer 4 keeps its fail-closed rule, and J-I is split: the hostile rules with no driver named `unset` keep the capability (item I), and the same configuration with such a driver is `review_checkout_unsafe` (item I2) although the canonical rule's `-filter` keeps the bytes (**Measured**). §14.5 states that conservatism as a rule: a configuration outside the one positively proven form is refused, and no byte probe stands in for the proof. §14.3 adds the precedence the two rules that read the same configuration key needed: part 1 is evaluated first, so a call in such a repository stops with `review_git_transform`, and `review_checkout_unsafe` is what evaluating §14.5 on its own gives — both fail closed and write nothing | §14.3, §14.5, §28 J I, I2, J, K, §34.7 |
| P2-IMPL-BLOCKER-002 (MID) — declared-base fixtures of §28 D and P-E were unreachable or read the wrong commit | §28 D's fourth moment required C-2(Kp) P12 to see a fact committed *after* Kp, which lies in a descendant of Kp and never in P, the only commit P12 reads. Its named factor — a hold of the Phase's Roadmap — is no declared-base fact of a RoadmapPlan at all (`phase_lifecycle` and `phase_state` derive from the Phase's own events, `state.py:209-210`, `state.py:289-301`), and for a Phase entry it was preempted by the live admission checks (blocker 006). §28 P-E's "Phase held" example had no reachable producer either: a Workline `phase-hold` of the entering Phase overlaps the pending planning mutation's scope and is refused by `MutationController.open` | the test contract separates reachable runtime windows from direct proof fixtures. Each kind uses a declared-base fact it actually reads; moment 4 states what that window really produces (the barrier holding the other operation's push while the continuation finishes, or a remote-less commit on top of Kp); and P12 and CP7 are exercised by fixtures whose P itself holds the changed fact, with the pre-Kp proof bypassed. §15.3 states that P12 reads P and only P, as the defence in depth it is | §15.3, §28 D, §28 P-E |
| P2-IMPL-BLOCKER-003 (MID) — the round trip could preempt the declared-base classification | §15.1 step 2 compares a result that carries the R9 selection from HEAD's committed basis, so a committed declared-base change that also changes that selection raised `review_roundtrip_mismatch` before step 3 could classify it — against §13 ("classified once, as a declared-base difference, and never also as an R9 mismatch"), §21.1 row 20 and §28 P-E, which name the pre-Kp currency proof as its owner | the declared-base precedence guard: step 2 compares the declared base on HEAD's committed view first, with the same function §13 item 3 uses, and where it differs the round trip classifies nothing and step 3's pre-Kp currency proof does. With the declared base equal, step 2 and `review_roundtrip_mismatch` are exactly what they were, and no new currency check exists | §13, §15.1, §25.1, §28 P-E |
| P2-IMPL-BLOCKER-004 (MID) — "one path-limited history read" contradicted §18.7's check points | §18.7 evaluates the barrier at three points, on two different commits for a normal push — HEAD before the stage holding the push is recorded, and the commit that stage makes — and permits reuse only within one commit ID. §28 K and §18.8 asked a legacy-only push for exactly one read | the cost is stated per distinct commit ID evaluated: a legacy-only push runs one fast-path read per commit the barrier evaluates — two for a normal push, the repeated evaluation of the commit made reusing its result — and no registered-Run discovery, no committed planning proof, no Review evaluation and no Git minimum. No check point is removed, and §18.7 is unchanged | §2, §18.8, §28 K, §33.2 |
| P2-IMPL-BLOCKER-005 (MID) — §7.7 did not decide an explicit entry outside the ties | the table gave `review_entry_ambiguous` for an explicit entry among the ties of an ambiguous selection and `review_entry_not_canonical` for an explicit B against a unique A, but said nothing about an explicit, startable Work outside the ties. Both codes were defensible readings, so the implementation had to choose one | the ambiguous selection decides: every explicit entry under it is `review_entry_ambiguous`, among the ties or not, because the committed basis names no canonical first Work. `review_entry_not_canonical` is reserved for a unique canonical A with an explicit B ≠ A. The missing row is in the table | §7.7, §28 D |
| P2-IMPL-BLOCKER-006 (HIGH) — a Roadmap hold and a pending Phase entry could deadlock | after Kp and before C-2(Kp), a disjoint-scope `roadmap-hold` applies its event and is held by the publication barrier (§18.7, §21.1 row 25), leaving the Roadmap held in the working tree. The same-request Phase-entry retry then met the live `roadmap_held` admission check, which §5.6 required to run on every call ("Every other check of steps 2 and 6 always runs"), so Kp was never proven, Km never made and the hold never publishable; `resume_roadmap` was refused by the pending hold's own scope, and `hold_roadmap` by the barrier again. Two valid Workline operations waited for each other with no path out but manual Git — while §18.7 and §21.1 row 25 promise that the held operation continues once Km is published | Phase-entry continuation mode (§5.8): once the slot's pending review-v1 Phase-entry planning mutation holds a Run with a canonical generation 1, the mutable facts that decide whether a *new* entry may begin no longer refuse that request. Every static and structural check still runs, a call that is not that request is still refused at the same-request check, and what changed is classified by currency, the use check, the pre-Kp proof, C-2(Kp) and the committed planning proof — on committed state, so an uncommitted effect of a held-back operation is never currency. §5.6's "always runs" becomes four exact branches, and §21.1 rows 23 and 25 state the progress path | §3, §4, §5.6, §5.8, §11.8, §13, §21.1, §21.3, §22, §25.1, §26.2, §27, §28 D, P-E, R, T |

Dependent changes: the status line and the header block; the §1 baseline row; the terms `canonical generation 1` and Phase-entry continuation mode (§3); the decision-summary row (§4); the §28 introduction and its new section T; the quality gates, evidence, checkpoint and cross-finding sections (§30, §31, §32, §34.8).

No new record, field, schema, exception, code or reason: continuation mode is an admission branch read from records that already exist, the precedence guard reuses the declared-base comparison of §13, and the other four repairs are wording that makes frozen rules say what they always meant.

Preserved:
- **P2-CONTRACT-001 to -008, P2-READY-001 to -004 and P2-READY-029 stay closed**; §34.8 checks each against this repair.
- Candidate 7 RETAIN; architecture reopen No; HUMAN-1 = A; HUMAN None.
- Review stays a subordinate authorization and quality gate and never a lifecycle controller; the Roadmap operation stays the only top-level owner and owns the new branch; `state.py` reads no Review record.
- Legacy planning is unchanged: no legacy invocation enters continuation mode, runs the guard, evaluates a checkout layer, or pays more than the fast path it paid before.
- The generation chain, the proof and publication chain, W and E, the committed R9 basis, the publication barrier, Planning Consumption v2 and same-Run recovery are untouched.
- P3 NOT STARTED, and no Work-terminal Review activation.
...
2. the live checks up to the request identity (since the implementation blocker repair, with their mutable admission facts suspended for a continuation, §5.8);
3. the canonical-input preflight;
4. the same-request and marker checks;
5. the remaining live acceptance checks, under the live interrupted-entry rule (and, since the implementation blocker repair, under the continuation branch of §5.8 where a canonical generation 1 exists);
...
### 34.8 Implementation blocker repair round 1 check against the closed findings

| closed finding | what this repair changes there |
| --- | --- |
| 001 C-2(Km) and the barrier | nothing weakened. No check point is removed, and no history publishes that did not publish before; only the cost is stated per evaluated commit (blocker 004). The barrier's own promise — that a held operation continues once Km is published — becomes reachable, because the planning operation can always get to Km (blocker 006) |
| 002 generation 4 and the Supersession | nothing. Continuation mode reaches generation 4 by no new path: a stale declared base after the seal is still found by the use check, which the continuation runs (§13.3, §28 T G) |
| 003 base and currency | strengthened in its own direction, never weakened. The declared base is still read from the committed view of the exact base commit, and it now also takes precedence over the R9-carrying round trip (blocker 003). P12 still proves currency on Kp's exact parent, and §15.3 now says so explicitly (blocker 002) |
| 004 checkout capability | stated, not loosened: layer 4's driver condition refuses even a configuration whose bytes a probe shows to be kept (blocker 001), and form L is still no proof by itself |
| 005 runtime-loss recovery | nothing. Continuation mode is read from a *pending* planning mutation, so a call with none — every runtime-loss call — keeps the order, the discovery and the classification of §12 exactly (§5.8, §28 T C) |
| 006 exact physical projection | nothing: E, the record check, P3 and CP5 are untouched, and a continuation registers through the same proofs |
| 007 canonical writer input | nothing: W is still computed from the canonical Candidate alone |
| 008 canonical-input preflight | nothing: it still runs before `_open`, in its frozen place, for every review-v1 call, a continuation included (§5.8) |
| READY-001 committed R9 basis | kept: R9 is still evaluated on the committed basis everywhere, and the round-trip guard exists precisely so that a declared-base change is never reported as an R9 mismatch (§7.7, §13, §15.1) |
| READY-002 pre-freeze resume setup | nothing: continuation mode begins at a canonical generation 1, and the pre-freeze resume setup governs only a mutation that has started no generation mutation. The two never overlap |
| READY-003 Phase-entry order | not reopened: the order of §5.6 is unchanged, step 7 still runs discovery only with no pending planning mutation, and the branch decides which checks refuse for one already-canonical Run, never when they run |
| READY-004 checkout proof | not reopened: the four layers stand, and blocker 001 makes layer 4's refusal explicit rather than weaker |
| READY-029 Git minimums | nothing: both thresholds, the fast path and the capability refusal are unchanged; blocker 004 only counts the fast-path reads |

**The questions this repair must answer.**

1. *Does continuation mode give Review lifecycle authority?* **No.** It reads two canonical facts to tell a new entry from a continuation, writes nothing and changes no lifecycle. Lifecycle still comes from events through `state.py`, which reads no Review record (§22 items 1 and 12).
2. *Does a mutable working-tree hold become committed currency?* **No.** Every currency evaluation and every proof reads the committed view of an exact commit (§13, §15.4), so an applied but uncommitted effect is invisible to all of them (§5.8, §28 T H).
3. *Can a fresh Phase entry bypass a held or inactive state?* **No.** With no pending planning mutation there is no continuation, and every live admission check refuses as today (§5.6 branches A and D, §28 T C).
4. *Can a pre-generation-1 pending Phase entry bypass it?* **No.** The boundary is a committed, readable generation 1; before it, branch A applies (§5.8, §28 T C, E).
5. *Can a same-request continuation after canonical generation 1 be blocked by the very Roadmap hold that is itself waiting at the publication barrier?* **No.** That is the deadlock this repair closes: the hold's uncommitted event no longer refuses the continuation, the continuation reaches Km, and the hold then publishes (§5.8, §21.1 row 25, §28 T A).
6. *Does a committed Roadmap, Phase or dependency change still reach currency, the use check and the proofs?* **Yes**, at the boundary the flow has reached, exactly as §13, §17, §15.6, §15.3 and §18.8 say (§5.8, §28 T G).
7. *Does a call with no pending planning mutation still use the P2-READY-003 order?* **Yes**, unchanged, canonical recovery discovery included (§5.6, §11.8, §12.2).
8. *Does legacy Phase entry change?* **No.** It never enters continuation mode, and its interrupted-entry and abandonment rules are untouched (§5.6 branch C, §28 T I).
9. *Does blocker 001 reopen P2-CONTRACT-004?* **No.** It keeps layer 4 exactly as it is and states why a configuration whose bytes survive is still refused; nothing refused becomes accepted (§14.5, §33.7).
10. *Does blocker 003 change R9 authority?* **No.** Roadmap-owned code still evaluates the selection on the committed basis, and the guard decides only which layer classifies a declared-base change (§7.7, §13, §15.1).

Lifecycle: nothing in this repair reads or writes a lifecycle fact — the continuation branch, the precedence guard, the R9 row, the checkout wording and the read-count wording decide refusals and test expectations only (§22). Review gains no authority: the Roadmap operation owns the entry branch and the guard, and `skills/review` keeps only the meaning of the records and the proofs (§26).
